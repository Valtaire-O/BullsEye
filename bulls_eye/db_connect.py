...
class BaseQuery:
    ...
    def __init__(self, testing=False):
        ...
        self.curr.execute(f"""select * from start_set""")
    ...
    def verify_img_data(self):
        ...
        data = self.curr.execute(
            f""" select {self.query_columns}   from start_set where done=False  and image!='n/a' """)
    ...
    def update_found(self, records, verification: bool):
        for r in records:
            ...
            self.curr.execute(
                f"""update start_set set found=True,done=True, needs_patch=True, image= %s where stage_id='{stage_id}'""",
                (image))
        self.config.commit()
        return 0
    ...
    def update_none_found(self, records, verification: bool):
        for r in records:
            ...
            if verification is True:
                self.curr.execute(
                    f"""update start_set set image='{none_found}',  `found` =False, done=True where stage_id = '{stage_id}'""")
                self.config.commit()
            else:
                self.curr.execute(
                    f"""update start_set set image='{none_found}', done=True, `found`=False, needs_patch=True where stage_id='{stage_id}'""")
        self.config.commit()
    ...
    def update_live_staging(self):
        ...
        for f in found_data:
            stage_id = f['stage_id']
            image = f['image']
            self.supabase.table(self.stage_tb).update({"prod_image": image, "image_check": True}).eq("int_id",
                                                                                                     stage_id).execute()
    ...
    def select_processed_records(self):
        found_data = self.curr.execute(
            f""" select {self.query_columns}   from start_set where done=True and found=True and needs_patch =True and image!='n/a' """)
        none_found_data = self.curr.execute(
            f""" select {self.query_columns}  from start_set where done=True and found=False and needs_patch =True and image='n/a' """)
    ...
    def select_live_staging(self):
        ...
        data_set = self.supabase.table(self.stage_tb).select("int_id", "Name", "mainURL").eq("image_check",
                                                                                             "False").execute()
